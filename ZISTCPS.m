%ZISTCPS ;ISF/RWF - DEVICE HANDLER TCP/IP SERVER CALLS ; 12/4/16 3:27pm
...
LONT2 F  U NIO R *NEWCHAR:30 S EXIT=$$EXIT Q:$T!EXIT
 ...
 J CHILDONT^%ZISTCPS(NIO,RTN,TLSACTIVE):(:16::):10 S %ZA=$ZA
 I %ZA\8196#2=1 W *-2 ;Job failed to clear bit
 G LONT2
 ;
CHILDONT(IO,RTN,TLSACTIVE) ;Child process for OpenM
 S $ETRAP="D ^%ZTER L  HALT",IO=$ZU(53)
 I 'TLSACTIVE U IO:(::"-M") ;Work like DSM - 160720 JLI/SMH modified for TLS
 I TLSACTIVE U IO:(::"-M":/SSL="TLS") ;160720 JLI/SMHWork like DSM with TLS
 S NEWJOB=$$NEWOK
